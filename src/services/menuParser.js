...
/**
 * Parse a menu file (PDF or image) in the browser
 */
...
function inferRestaurantType(text, structure) {
  ...
  if (textLower.includes('fine dining')) return 'fine-dining';
  if (textLower.includes('cafe') || textLower.includes('coffee')) return 'cafe';
  if (textLower.includes('diner')) return 'diner';
  if (textLower.includes('bistro')) return 'bistro';
  if (structure.items.length < 15) return 'specialized';
...
function identifyStrengths(structure) {
  ...
  if (structure.items.length >= 10 && structure.items.length <= 30) {
    strengths.push('Appropriate menu size');
  }
